...
function renderObjetivos(rows) {
  ...
  if (!mFact && !mAum) {
    ...
    if (mFact && factAn < mFact) {
      ...
      const ticketPromedio = activos > 0 ? factAn / activos : 0;
      const clientesExtraFact = ticketPromedio > 0 ? Math.ceil(faltaFact / ticketPromedio) : '?';
      insights.push(`Para alcanzar la meta de facturación necesitás USD ${fmtUSD(faltaFact)} adicionales. Con el ticket promedio actual de USD ${fmtUSD(ticketPromedio)} por cliente activo, se necesitarían ${clientesExtraFact} clientes nuevos activos.`);
